  ...
  methods: {
    ...
    async login() {
      if (this.userName === '' && this.password === '') {
        ...
        let json = await login(this.userName, this.password)
        console.log(json)
        if (json.state === 200) {
          localStorage.setItem('token', json.data.token)
          this.$cookies.set('name', json.data.name)
          this.$cookies.set('avatar', json.data.avatar)
          this.$cookies.set('cinemaId', json.data.cinemaId)
          Message({
            message: '登录成功',
            type: 'success'
          })
          this.$router.push({ path: '/home', query: { name: json.data.name } })
        } else {
          Message({
            message: '登录失败账号密码错误',
            type: 'error'
