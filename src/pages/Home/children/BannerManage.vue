  ...
  <div id="movie-manage">
    ...
    <div class="banner-table">
      <el-table border :data="tableData">
        <el-table-column label="广告 ID" align="center" width="100" prop="id"> </el-table-column>
        <!-- <el-table-column label="图片 URL" show-overflow-tooltip prop="img"> </el-table-column> -->
        <el-table-column label="图片" align="center" width="375">
          <template slot-scope="scope">
            <el-popover placement="top-start" title="" trigger="hover">
              <img :src="scope.row.img" alt="" style="width: 375px; height: 100px" />
              <img slot="reference" :src="scope.row.img" style="width: 375px; height: 100px" />
            </el-popover>
          </template>
        </el-table-column>
        <!-- <el-table-column label="跳转 URL" show-overflow-tooltip prop="url"> </el-table-column> -->
        <el-table-column label="跳转的电影id" show-overflow-tooltip prop="movieId"></el-table-column>
    ...
    <div>
      <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible" :modal-append-to-body="false" v-if="dialogFormVisible" :showClose="false">
        <el-form :label-position="labelPosition" :rules="rules" label-width="80px" :model="bannerInfo">
          ...
          <el-form-item label="跳转电影" prop="firstTitle">
            <el-col :span="16">
              <el-input v-model="bannerInfo.movieId"></el-input>
            </el-col>
          </el-form-item>
        </el-form>
...
<script>
import { getBanner, updateBannerInfo, upLoadFile, deleteBannerInfo } from '@/api/index.js'
...
export default {
  ...
  methods: {
    async loadCurrentPageBanner(pageNum, limit, input) {
      ...
      if (json.state === 200) {
        console.log(json.data.beanList);
    ...
    async manageBannerInfo() {
      if (this.bannerInfo.img) {
        if (this.$refs.uploadImg.files[0]) {
          ...
          if (json.state === 200) {
            if (json.data) {
              this.bannerInfo.img = 'https://liyuxiang.com.cn/' + json.data.img
            }
          } else {
