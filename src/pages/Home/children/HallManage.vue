  ...
  <div id="hall-manage">
    ...
    <div class="hall-table">
      <el-table border :data="tableData">
        <el-table-column label="影厅 ID" align="center" width="100" prop="id"> </el-table-column>
        <el-table-column label="影院名" width="250" prop="cinemaNm"> </el-table-column>
        <el-table-column label="影厅名" show-overflow-tooltip prop="hallType"> </el-table-column>
        <el-table-column width="100" align="center" label="影厅座位">
          <template slot-scope="scope">
            <el-button size="mini" type="info" @click="handleSeat(scope.$index, scope.row)">座位</el-button>
          </template>
        </el-table-column>
    ...
    <!--添加影厅-->
    <div>
      <el-dialog title="添加影厅" :visible.sync="addDialogFormVisible" :modal-append-to-body="false" v-if="addDialogFormVisible" :show-close="false">
        <el-form :label-position="labelPosition" :rules="rules" label-width="80px" :model="hallInfo">
          <el-form-item label="影院名" prop="cinemaNm">
            <el-col :span="12">
    ...
    <!--影厅座位-->
    <div>
      <el-dialog title="座位信息" :visible.sync="seatDialogFormVisible" :modal-append-to-body="false" v-if="seatDialogFormVisible" :show-close="false">
        <div class="seat">
    ...
    <!-- loading -->
    <div>
      <el-dialog title="座位生成中..." :visible.sync="loadingDialogFormVisible" :modal-append-to-body="false" v-if="loadingDialogFormVisible" :show-close="false">
...
import { getCurrentPageHall, deleteHall, updateHallInfo, addHall, getOptions, getSeats, commitSeat } from '@/api/index.js'
export default {
  name: 'HallManage',
  data() {
    return {
      ...
      rules: {
        addcinemaId: [{ required: true, message: '影院名不能为空', trigger: 'change' }],
        addHallTypeId: [{ required: true, message: '影厅名不能为空', trigger: 'blur' }]
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    async setSeat() {
      ...
      for (let i = 0; i < this.ySeat; i++) {
        ...
        totalArray.push(xArray)
      }
      this.seat = totalArray
      if (this.seats.length > 0) {
        for (let i = 0; i < this.seats.length; i++) {
          let tmp = {}
          tmp.yCoord = this.seats[i].yCoord
          tmp.xCoord = this.seats[i].xCoord
          tmp.id = this.seats[i].id
          if (this.seats[i].type == 'danren') {
            tmp.iconSrc = 'seatPre.png'
            tmp.type = 'danren'
          } else if (this.seats[i].type == 'road') {
            tmp.iconSrc = 'noSeat.png'
            tmp.type = 'road'
          }
          this.seat[this.seats[i].yCoord - 1][this.seats[i].xCoord - 1] = tmp
        }
      }
    },
