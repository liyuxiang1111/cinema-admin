  ...
  <div id="movie-manage">
    ...
    <div class="movie-table">
      <el-table border :data="tableData">
        <el-table-column label="电影 ID" align="center" width="70" prop="id"> </el-table-column>
        <el-table-column label="电影名" width="90" show-overflow-tooltip prop="nm"> </el-table-column>
        <el-table-column label="英文名" show-overflow-tooltip prop="enm"> </el-table-column>
        <el-table-column label="导演" align="center" show-overflow-tooltip prop="dir"> </el-table-column>
        <el-table-column label="版本" width="90" prop="version"> </el-table-column>
        <el-table-column label="类型" align="center" width="100" prop="cat"> </el-table-column>
        <el-table-column label="上映时间" width="110" prop="rt"> </el-table-column>
        <el-table-column label="想看人数" prop="wish"> </el-table-column>
        <el-table-column label="观看人数" prop="watched"> </el-table-column>
        <el-table-column label="状态" prop="showst" :formatter="formatShowst"> </el-table-column>
    ...
    <div class="block" v-if="tableData.length">
      <el-pagination @current-change="currentChange" background layout="prev, pager, next" :page-size="8" :page-count="total"> </el-pagination>
    </div>
    <!--弹框-->
    <div>
      <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible" :modal-append-to-body="false" v-if="dialogFormVisible" :showClose="false">
          ...
          <el-form-item label="上映地区" prop="src">
            <el-col :span="16">
              <el-select v-model="movieInfo.src" placeholder="请选择地区" style="width: 100%">
                <el-option v-for="item in srcOptions" :key="item.value" :label="item.label" :value="item.value"> </el-option>
              </el-select>
            </el-col>
          </el-form-item>
          ...
          <el-form-item label="状态" prop="showst">
            <el-col :span="16">
              <el-select v-model="movieInfo.showst" placeholder="请选择语言" style="width: 100%">
                <el-option v-for="item in movieShowst" :key="item.value" :label="item.label" :value="item.value"> </el-option>
              </el-select>
            </el-col>
          </el-form-item>
          ...
          <el-form-item label="宣传海报" prop="photos">
            <el-row :span="16">
              <ul class="upload-imgs">
                <div v-if="isNew">
                  <li v-for="(value, index) in newPhotos" :key="index">
                    <p class="img"><img :src="value" width="150px" height="90px" /><a class="close" @click="delImg2(index)">×</a></p>
                  </li>
                </div>
                <div v-if="!isNew">
                  <li v-for="(value, index) in movieInfo.photosList" :key="index">
    ...
    <!-- loading -->
    <div>
      <el-dialog title="电影添加中..." :visible.sync="loadingDialogFormVisible" :modal-append-to-body="false" v-if="loadingDialogFormVisible" :show-close="false">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        nm: [{ required: true, message: '电影名不能为空', trigger: 'blur' }],
        img: [{ required: true, message: '请上传电影海报', trigger: 'blur' }],
        dir: [{ required: true, message: '导演信息不能为空', trigger: 'blur' }],
        star: [{ required: true, message: '主演信息不能为空', trigger: 'blur' }],
        dur: [{ required: true, message: '片长信息不能为空', trigger: 'blur' }],
        language: [{ required: true, message: '请选择语言', trigger: 'change' }],
        cat: [{ required: true, message: '请选择电影类型', trigger: 'change' }],
        rt: [{ required: true, message: '请选择上映日期', trigger: 'blur' }],
        dra: [{ required: true, message: '电影简介信息不能为空', trigger: 'blur' }]
      },
      movieShowst: [
        {
          value: 1,
    ...
    async addImg(e) {
      ...
      if (json.state == 200) {
        ...
        if (this.dialogTitle === '编辑电影信息') {
          this.movieInfo.photosList.push('https://liyuxiang.com.cn/' + json.data.img)
        } else {
          this.newPhotos.push('https://liyuxiang.com.cn/' + json.data.img)
        }
      }
      ...
      if (
        ...
        if (json.state === 200) {
          if (this.movieInfo.rt) {
            ...
            this.movieInfo.rt =
              date.getFullYear() + '-' + (date.getMonth() + 1 >= 10 ? date.getMonth() + 1 : '0' + (date.getMonth() + 1)) + '-' + (date.getDate() >= 10 ? date.getDate() : '0' + date.getDate())
          }
          if (json.data) {
            if (json.data.img) this.movieInfo.img = 'https://liyuxiang.com.cn/' + json.data.img
            if (json.data.videoImg) this.movieInfo.videoImg = 'https://liyuxiang.com.cn/' + json.data.videoImg
            if (json.data.video) this.movieInfo.videoUrl = json.data.video
          }
          //判断是编辑还是添加
          if (this.dialogTitle === '编辑电影信息') {
