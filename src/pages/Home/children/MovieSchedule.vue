  ...
  <div id="movie-schedule">
    ...
    <div class="schedule-table">
      <el-table border :data="tableData" style="width: 100%">
        <el-table-column label="排片 ID" align="center" width="100" prop="id"> </el-table-column>
        <el-table-column label="电影名" show-overflow-tooltip prop="movieNm"> </el-table-column>
        <el-table-column label="图片" align="center" width="60">
          <template slot-scope="scope">
            <el-popover placement="top-start" title="" trigger="hover">
              <img :src="scope.row.movieImg" alt="" style="width: 150px; height: 150px" />
              <img slot="reference" :src="scope.row.movieImg" style="width: 30px; height: 30px" />
            </el-popover>
            <span>{{ scope.row.title }}</span>
          </template>
        </el-table-column>
        <el-table-column label="放映大厅" prop="hallNm"> </el-table-column>
        <el-table-column label="放映日期" prop="days.day"> </el-table-column>
        <el-table-column label="放映日期" prop="days.day"> </el-table-column>
        <el-table-column label="放映时间" prop="time"> </el-table-column>
    ...
    <!--添加排片-->
    <div>
      <el-dialog title="添加排片" :visible.sync="addDialogFormVisible" :modal-append-to-body="false" v-if="addDialogFormVisible" :show-close="false">
          ...
          <el-form-item label="电影名" prop="movieId">
            <el-col :span="12">
              <el-select v-model="scheduleInfo.movieId" placeholder="请选择" @change="changeMovie" style="width: 100%">
                <el-option v-for="item in movieOptions" :key="item.id" :label="item.nm" :value="item.id"> </el-option>
              </el-select>
            </el-col>
          </el-form-item>
          <el-form-item label="影院" prop="cinemaId">
            <el-col :span="12">
              <el-select v-model="scheduleInfo.cinemaId" placeholder="请选择" @change="changeCinema" style="width: 100%">
                <el-option v-for="item in cinemaOptions" :key="item.id" :label="item.nm" :value="item.id"> </el-option>
              </el-select>
            </el-col>
          </el-form-item>
          <el-form-item label="影厅" prop="hallId">
            <el-col :span="12">
              <el-select v-model="scheduleInfo.hallId" placeholder="请选择" style="width: 100%">
                <el-option v-for="item in hallOptions" :key="item.id" :label="item.id + ' 号厅'" :value="item.id"> </el-option>
              </el-select>
            </el-col>
          </el-form-item>
          <el-form-item label="上映日期" prop="public_date">
            <el-col :span="12">
              <el-date-picker v-model="moviePublicDate" type="date" readonly style="width: 100%"> </el-date-picker>
            </el-col>
          </el-form-item>
          <el-form-item label="放映日期" prop="movieShowDate">
            <el-col :span="12">
              <el-date-picker v-model="scheduleInfo.movieShowDate" type="date" placeholder="选择日期" :picker-options="datePickerOptions" value-format="yyyy-MM-dd" style="width: 100%">
              </el-date-picker>
            </el-col>
          </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      searchInput: '',
      rules: {
        movieId: [{ required: true, message: '电影名不能为空', trigger: 'change' }],
        cinemaId: [{ required: true, message: '影院名不能为空', trigger: 'change' }],
        hallId: [{ required: true, message: '影厅名不能为空', trigger: 'change' }],
        movieShowDate: [{ required: true, message: '放映日期不能为空', trigger: 'change' }],
        movieShowTime: [{ required: true, message: '放映时间不能为空', trigger: 'change' }],
        price: [{ required: true, message: '售价不能为空', trigger: 'blur' }, { validator: checkPrice, trigger: 'blur' }]
      },
  ...
  methods: {
    ...
    async addScheduleInfo() {
      ...
      if (
        this.scheduleInfo.movieId &&
        this.scheduleInfo.cinemaId &&
        this.scheduleInfo.hallId &&
