  ...
  <div id="movie-manage">
    ...
    <div class="movie-table">
      <el-table border :data="tableData">
        <el-table-column label="小吃 ID" align="center" width="100" prop="id"> </el-table-column>
        <el-table-column label="所属影院" align="center" width="220" prop="cinemaNm"> </el-table-column>
        <el-table-column label="套餐名称" show-overflow-tooltip prop="firstTitle"> </el-table-column>
        ...
        <el-table-column label="详细信息" show-overflow-tooltip prop="secondTitle"> </el-table-column>
        <el-table-column label="价格" align="center" show-overflow-tooltip prop="price"> </el-table-column>
        <el-table-column label="已购数量" prop="curNumber"> </el-table-column>
        <el-table-column label="剩余数量" align="center" width="100" prop="totalNumber"> </el-table-column>
...
<script>
import { getSnacks, updateSnackInfo, upLoadFile, deleteSnackInfo, getOptions } from '@/api/index.js'
...
export default {
  ...
  data() {
    return {
      ...
      cinemaList: [],
      rules: {
        imageUrl: [{ required: true, message: '小吃图片不能为空', trigger: 'blur' }],
        firstTitle: [{ required: true, message: '套餐名称不能为空', trigger: 'blur' }],
        secondTitle: [{ required: true, message: '详细信息不能为空', trigger: 'blur' }],
        price: [{ required: true, message: '价格不能为空', trigger: 'blur' }],
        totalNumber: [{ required: true, message: '总数不能为空', trigger: 'blur' }]
      }
    }
  },
  created() {
    ...
    async manageSnackInfo() {
      if (this.snackInfo.imageUrl && this.snackInfo.firstTitle && this.snackInfo.secondTitle && this.snackInfo.price && this.snackInfo.totalNumber) {
        if (this.$refs.uploadImg.files[0]) {
          ...
          if (json.state === 200) {
            if (json.data) {
              this.snackInfo.imageUrl = 'https://liyuxiang.com.cn/' + json.data.img
            }
          } else {
